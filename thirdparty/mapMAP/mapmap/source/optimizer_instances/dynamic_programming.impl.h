    ...
    tbb::parallel_do(queue.begin(), queue.end(),
        [&](const luint_t n, tbb::parallel_do_feeder<luint_t>& feeder)
        {
			...
			for (luint_t i = 0; i < dpb.c_node.dependency_degree; ++i) {
				...
				//if (dpb.c_dep_costs[i] == NULL) {
				//	std::cout << i << std::endl;
				//	//std::cout << dpb.c_node.dependency_edge_ids[0] << std::endl;
				//	std::cout << dpb.c_node.dependency_degree << std::endl;
				//	//std::cout << this->m_cbundle->m_graph->edges().size() << std:£ºendl;
				//	system("pause");
				//}
			}
